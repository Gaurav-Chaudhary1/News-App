...
import com.example.mynewsapp.databinding.FragmentHeadlinesBinding
...
class HeadlinesFragment : Fragment(R.layout.fragment_headlines) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        newsAdapter.setOnClickListener {
            val article = it
            // Check if the article has a valid source
            if (article.source?.name != null && article.url != null) {
                val bundle = Bundle().apply {
                    Log.d("HeadlinesFragment", "Article source: ${article.source?.name}, URL: ${article.url}")
                    putSerializable("article", article)
                }
                findNavController().navigate(R.id.action_headlinesFragment2_to_articleFragment, bundle)
            } else {
                Toast.makeText(context, "Article is missing data", Toast.LENGTH_SHORT).show()
            }
        }
